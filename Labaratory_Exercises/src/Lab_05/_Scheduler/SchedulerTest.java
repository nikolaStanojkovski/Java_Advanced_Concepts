...
class Timestamp<T> implements Comparable<Timestamp> {
    ...
    public String toString() {
        return String.format("%s %s", this.getTime(), this.getElement());
    }
}
...
class Scheduler<T> {
    ...
    public Timestamp<T> next() {
        return this.timestamps.stream().filter(i -> i.getTime()
    ...
    public List<Timestamp<T>> getAll(LocalDateTime begin, LocalDateTime end) {
        return this.timestamps.stream().filter(i -> i.getTime().isAfter(begin) &&
                i.getTime().isBefore(end)).collect(Collectors.toList());
    }
